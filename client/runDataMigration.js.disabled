...
const migrateAPreview = async () => {
  ...
  await Meteor.callAsync(
    ...
    (err, pattern) => {
          ...
          Jimp.read(Buffer.from(base64Image, 'base64'), (err, image) => {
            ...
            image
              ...
              .getBase64(Jimp.AUTO, (err, res) => {
                Meteor.callAsync('patternPreview.save', { _id, uri: res });
              });
          });
        });
      } else {
      ...
      Meteor.callAsync('migrations.deleteAutoPreview', _id); // clear out the old entry from the pattern data in the Patterns collection
    },
  );
...
const migratePreviews = async () => {
  console.log('starting to migrate pattern previews');
  Meteor.callAsync('migrations.getPatternIds', (err, ids) => {
