...
if (Meteor.isServer) {
  ...
  // Only create the S3 directive if required AWS env vars are present. This avoids
  // startup crashes when AWS credentials are not configured (e.g., in local dev).
  const hasAwsEnv = process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY && process.env.AWS_BUCKET;
  if (!hasAwsEnv) {
    console.warn('[slingshot] AWS env vars not found; skipping S3 directive "myImageUploads". To enable uploads, set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY and AWS_BUCKET in your environment.');
  } else {
    try {
      ...
      console.info('[slingshot] S3 upload directive "myImageUploads" created.');
    } catch (err) {
      console.error('[slingshot] Failed to create Slingshot directive "myImageUploads":', err);
    }
  }
}
