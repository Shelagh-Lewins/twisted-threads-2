...
describe('ImageUploader preview-before-upload', () => {
  ...
  it('shows preview and confirm/cancel after image selection', async () => {
    ...
    const input = screen.getByLabelText('Close').parentElement.querySelector('input[type="file"]');
  ...
  it('shows error for non-image file', async () => {
    setup();
    const file = new File([new ArrayBuffer(10)], 'test.txt', { type: 'text/plain' });
    const input = screen.getByLabelText('Close').parentElement.querySelector('input[type="file"]');
    fireEvent.change(input, { target: { files: [file] } });
    await waitFor(() => {
  ...
  it('shows error for file >2MB', async () => {
    setup();
    const file = new File([new ArrayBuffer(3 * 1024 * 1024)], 'big.png', { type: 'image/png' });
    const input = screen.getByLabelText('Close').parentElement.querySelector('input[type="file"]');
    fireEvent.change(input, { target: { files: [file] } });
    await waitFor(() => {
