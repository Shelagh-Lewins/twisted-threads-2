...
class WeavingDesignBorder extends PureComponent {
	...
	handleClickWeavingCell(rowIndex, tabletIndex) {
		...
		if (editMode === 'direction') {
			dispatch(editBorderWeavingCell({
				_id: pattern._id,
				side,
				'row': rowIndex,
				'tablet': tabletIndex,
			...
			dispatch(editBorderWeavingCellTurns({
				_id: pattern._id,
				side,
				'row': rowIndex,
				'tablet': tabletIndex,
		...
		return (
			<div className="controls">
				{isEditing
					...
					: <Button color="primary" onClick={this.toggleEditWeaving}>{`Edit ${label} weaving design`}</Button>}
			</div>
		);
	}
	...
	renderToolbar() {
		const { numberOfRows } = this.props;
		const { controlsOffsetX, controlsOffsetY, editMode, numberOfTurns } = this.state;
	...
	render() {
		...
		return (
			<div className={`weaving border-weaving border-weaving-${side} ${isEditing ? 'editing' : ''}`}>
