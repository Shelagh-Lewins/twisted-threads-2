...
export default function FreehandPreviewSVG({
  ...
  const colorValue = threadColor === -1 ? 'transparent' : palette[threadColor];
  ...
  if (rowIndex !== 0) {
    ...
    previousColor =
      previousCell.threadColor === -1
        ? 'transparent'
        : palette[previousCell.threadColor];
  } else if (freehandChart[rowIndex + 1]) {
    // try next colour instead
    previousCell = freehandChart[rowIndex + 1][tabletIndex];
    previousColor =
      previousCell.threadColor === -1
        ? 'transparent'
...
FreehandPreviewSVG.propTypes = {
  freehandChart: PropTypes.arrayOf(PropTypes.any).isRequired,
