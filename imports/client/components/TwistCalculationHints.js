...
export default function TwistCalculationHints({
	...
	if (patternType !== "freehand") {
		...
		if (allExcluded) {
			...
			hints = (
				<span className="hint">
					Unable to determine whether the pattern will repeat or is twist
					neutral because all tablets have been excluded from twist calculations
					- check the threading chart.
				</span>
			);
		} else {
			const twistNeutralText = (
				<span className="hint">
					{patternIsTwistNeutral
						? "The pattern is twist neutral."
						: "The pattern is not twist neutral."}
				</span>
			);
			...
			if (patternWillRepeat) {
				if (printView) {
					...
					if (previewOrientation === "up" && canEdit) {
						repeatHint +=
							" To see repeats, set the woven band orientation to left or right.";
					}
				}
			}
			...
			const excludedTabletsText = anyExcluded ? (
				<span className="hint">
					Some tablets have been excluded from twist calculations - check the
					threading chart.
				</span>
			) : undefined;
