...
import { getPalette } from '../modules/pattern';
import FreehandSVG from './FreehandSVG';
import { DEFAULT_DIRECTION } from '../../modules/parameters';

import './FreehandThreads.scss';

function FreehandThreads(props) {
  const { palette, selectThread, selectedThread, threadColorIndex } = props;
    ...
    svgNames.map((svgName) => {
      ...
      return (
        <li
          ...
          tabIndex='0'
        >
          <FreehandSVG chartCell={chartCell} palette={palette} />
        </li>
      );
    });
