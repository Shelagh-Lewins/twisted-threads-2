...
class MainMenu extends Component {
  ...
  render() {
    const { history, isAuthenticated, selectedMainMenuItem } = this.props;

    const handleClickMenuItem = ({ url }) => {
      history.push(url);
    };

    const renderMenuItem = ({ name, sectionBreak, url, value }) => {
      let className = '';
    ...
    return <ul className='main-menu'>{renderMenuItems()}</ul>;
  }
}
