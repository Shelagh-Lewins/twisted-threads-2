...
import { editPaletteColor, getPalette } from '../modules/pattern';
import { getCanAddPatternImage, getCanCreateColorBook } from '../modules/auth';
...
class Palette extends PureComponent {
  ...
  handleClickRestoreDefaults = () => {
    ...
    const response = confirm(
      'Do you want to restore the default colour palette? This will delete your current working palette but will not affect your colour books.',
    ...
    return (
      ...
      >
        ...
        <span // eslint-disable-line jsx-a11y/control-has-associated-label
          ...
          tabIndex='0'
        >
          <SVGPaletteEmpty stroke={isEditing ? '#666' : '#000'} />
        </span>
      </label>
    );
  };
  ...
  renderEditColorPanel = () => {
    const { canCreateColorBook, colorBookAdded, colorBooks, dispatch } =
      this.props;
    const { editMode, colorValue, isEditingColorBook, pickerReinitialize } =
      this.state;
    ...
    return (
      <div className='controls'>
        ...
        <div className='toggle'>
          {isEditing ? (
            <Button color='primary' onClick={this.handleClickDone}>
              Done
            </Button>
          ) : (
            <Button color='primary' onClick={this.handleClickEdit}>
              Edit thread colors
            </Button>
          )}
        </div>
