...
class WeavingDesignAllTogether extends PureComponent {
  ...
  renderRowDirectionButton(direction, index) {
    ...
    return (
      <div className='weaving-instructions'>
        <div className='row-direction-buttons'>
          {weavingInstructions.map((direction, index) =>
            this.renderRowDirectionButton(direction, index),
          )}
        </div>
      </div>
    );
  ...
  render() {
    ...
    return (
      <div className={`weaving-all-together ${isEditing ? 'editing' : ''}`}>
        <div>
          Turn all tablets together, forward or backward, following the sequence
          shown below
        </div>
