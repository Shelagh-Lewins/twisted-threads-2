...
class People extends Component {
  ...
  componentDidMount() {
    const { dispatch, itemsPerPage, pageSkip } = this.props;
  ...
  componentDidUpdate(prevProps) {
    const { currentPageNumber, dispatch, itemsPerPage, pageSkip } = this.props;
  ...
  renderUsers() {
    ...
    if (!Array.isArray(users) || users.length === 0) {
      return <div className='empty'>There are no people to display</div>;
    }
