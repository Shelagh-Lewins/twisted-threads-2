...
class Set extends Component {
  ...
  render() {
    ...
    if (!isLoading) {
      if (set) {
        const { createdBy, description, name } = set;
                ...
                <Col lg='12'>
                  <div className='set-page-name'>
                    <span
                      className='icon'
                      style={{
                        backgroundImage: `url(${Meteor.absoluteUrl('/images/search_set.png')}`,
                      }}
                      title={`Set: ${name}`}
                    />
                    <EditableText
        ...
        content = (
          <Container>
            <p>
              Either this set does not exist or you do not have permission to
              view it
            </p>
          </Container>
        );
